...
Usage::
    ...
    python -m dbp_prediction.cli.train_mlp --seed 2024 --targets T_THMs_ug_L,DBCM_ug_L
"""
...
DEFAULT_OUT_PATH = CHECKPOINT_DIR / "mlp_checkpoint.pt"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train per-target baseline MLP models for DBP prediction")
...
def main() -> None:
    ...
    if getattr(args, "config", None):
        binding = bind_legacy_model_config(args.config, "mlp", DEFAULT_OUT_PATH)
    ...
    dropout = float(_get_arg(args, "dropout", model_params.get("dropout", 0.2)))
    activation = str(_get_arg(args, "activation", model_params.get("activation", "ReLU")))
